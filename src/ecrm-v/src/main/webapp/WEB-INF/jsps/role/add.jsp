...
<style type="text/css">
.bui-grid-table tr:hover{ background-color: #EBEFF2; }
</style>
</head>
<body>
<div class="demo-content" style="margin: 27px;">
	...
	$(function () {
		...
		$("#J_Form_Submit,#J_Form_Submit2").click(function(){
			...
			$.ajax({
				type: "POST",
				url: "${ctx}/PermissionRole/save",
				data:{'menucode' : submit, 'rolename' : $("#rolename").val()},
				async: false,
				dataType: "json",
			...
			var tree = new TreeGrid({
					...
					columns : [
						...
						{title : '是否显示',dataIndex :'menustatus',width:'25%',renderer:function(value,obj){
							return "<label class='checkbox lable-box'><input type='checkbox' name='menustatus' "+(value==1?"'checked'":"")+" level='"+obj.menulevel+"'/></label>";
						}}]
			});
			tree.render();
		});
