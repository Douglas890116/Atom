      ...
      <div style="float: left;margin-right: 126px;">
          ...
          <div class="control-group span7">
           ...
           <div class="control-group span8 toggle-display">
              <label class="control-label">上下分金额：</label>
                <input type="text" name="moneyinoutamountstart" style="width:60px;">-
                <input type="text" name="moneyinoutamountend" style="width:60px;">
            </div>
            <div class="control-group span7 toggle-display">
              <label class="control-label">备注：</label>
                <input name="moneyinoutcomment" type="text" data-tip='{"text":"备注"}' class="control-text"/>
          </div>
          
          <div class="control-group span7 toggle-display">
            	<label class="control-label">单号：</label>
            	<input name="orderno" type="text" data-tip='{"text":"单号"}' class="control-text"  value="${param.orderno }" placeholder="订单号"/>
            </div>
            
            <div class="control-group span7 toggle-display">
            	<label class="control-label">批次号：</label>
            	<input name="patchno" type="text" data-tip='{"text":"批次号"}' class="control-text"  value="${param.patchno }" placeholder="批次号"/>
            </div>
	   ...
	   function openTableUser(data){
        ...
        var Grid = BUI.Grid,
          ...
          columns = [
            ...
            { title: '备注',width: '20%',sortable: false,dataIndex: 'moneyinoutcomment', renderer:function(value,data){
            	if(value == "success") {
            		return value;
            	} else {
            ...
            { title: '批次号',width: '12%',sortable: false,dataIndex: 'patchno'},
            { title: '操作',width: 105,sortable: false,renderer:function(value,obj){
            	var temp = '';
            	//updatecapital=操作状态
            	//
            	if(obj.updatecapital != 1 &&obj.opreatetype==1&obj.backmoney==1){
            		
					temp = temp+'<button type="button" class="button button-danger" style="margin-right:9px;" onclick="LogicFunc.backLoseMoney(\''+obj.moneyinoutcode+'\',this)" ><span class="icon-repeat icon-white"></span>返还金额 </button>';
            	}else if(obj.backmoney==2) {
            		temp = temp+'已返还金额';
            	} else if(obj.updatecapital == 0) {
            		temp = temp+'请确认平台方订单状态后再冲正';
            	} else{
            		temp = temp +'<button type="button" class="button button-danger" style="margin-right:9px;visibility: hidden;"  ><span class="icon-repeat icon-white"></span></button>'
            	}
				return temp;
            }}
          ];
        var store = new Store({
            url : '${ctx}/moneyInAndOut/findMoneyInAndOut',
            autoLoad : false,
            pageSize:15,
            sortInfo : {
                    field : 'timesort',
