            ...
            <form id="objForm" method="post" class="form-horizontal">
	           		...
	           		<div class="controls">
	           			<select name="brandcode" aria-pressed="false" aria-disabled="false"   class="bui-form-field-select bui-form-field" style="width:295px;">
							...
							<c:forEach items="${brands}" var="brand">
								<option value="${brand.brandcode }">${brand.enterprisename } - ${brand.brandname }</option>
							</c:forEach>
	              		</select>
	           		</div>
	            </div>
                <div class="control-group">
                  <label class="control-label">公告标题：</label>
                    <div class="controls">
                        <input class="input-normal control-text" name="title" type="text" style="width:270px;" data-rules="{required:true,maxlength:50}" data-tip="{text:'标题'}"/>
                    </div>
                </div>
                <div class="control-group" style="width: 800px;height: 320px;margin-top: 15px;">
                  <label class="control-label">公告内容：</label>
                  <div class="controls">
                      <textarea style="width: 600px;height: 300px;margin: 0;padding: 15px;font-size: 14px;line-height: 28px;box-sizing: border-box;-o-box-sizing: border-box;-ms-box-sizing: border-box; -moz-box-sizing: border-box;-webkit-box-sizing: border-box;" name="content"></textarea>
                  </div>
                </div>
               <div class="control-group">
	              <label class="control-label">有效时间：</label>
	              <div class="controls bui-form-group" data-rules="{dateRange : true}">
	                <input name="begintime"  type="text" data-tip='{"text":"起始时间"}' class="calendar calendar-time"/>
	                <span>&nbsp;-&nbsp;</span><input  name="endtime" type="text" data-tip='{"text":"结束时间"}'  class="calendar calendar-time"/>
	              </div>
	           </div>
...
$(function(){
	...
	bindClick("J_Form_Submit","${ctx}/BrandNotic/save",form);
});
</script>
