            	...
            	<ul>
                	<li>
                       ...
                       <!--  	<div class="clear"></div>
                        <div class="wu_wan">
							...
							报表统计了前10个总代团队的游戏输赢，并汇总了其他总代团队的游戏输赢，按照百分比进行显示，悬浮显示具体输赢金额
							</a>
                        </div> -->
                    </li>
                	<li>
                     ...
                     <!--   	<div class="clear"></div>
                        <div class="wu_wan">
                        	<a href="##" style="display: block;width: 80%;margin: 0 auto;text-decoration: none;color:#0E8BB9;font-size: 14px;">
							报表统计了前10个企业品牌的游戏输赢，并汇总了其他品牌的游戏输赢，按照百分比进行显示，悬浮显示具体输赢金额
							</a>
                        </div> -->
                    </li>
                	...
                	<li class="bnoe">
                    	<p id="teamPerUseractivityCanvas" style="width: 100%;padding-top:45px;"></p>
                       	<div class="clear"></div>
                        <div class="wu_wan">
                        	<a href="##" style="display: block;width: 80%;margin: 0 auto;text-decoration: none;color:#0E8BB9;font-size: 13px;">
							报表统计了最后一次登录时间在三天以内,三天以上，一周以上，半个月以上，一个月以上，三个月以上的用户，各个周期之间不重复统计
							</a>
                        </div>
                    </li>
                </ul>
...
<script type="text/javascript">
	var teamreport = function(){
		...
		this.circle = function(picidselector,resultdata){
			$("#"+picidselector).html("");
			//alert(resultdata.message.title);
			//alert(resultdata.message.data);
			
			//var dtitle = eval("["+resultdata.message.title+"]");
		...
		this.render = function(date,data,bindelement){
			...
			if(data&&data.teamLoseWin){
				...
				this.getData("stat","teamLoseWin","${ctx}/Pandect/ELoseWin",date,bindelement);
			}
			// 充值总额
			if(data&&data.teamSaveMoney){
				this.stat("teamSaveMoney",data.teamSaveMoney);
			}else{
				this.getData("stat","teamSaveMoney","${ctx}/Pandect/ESaveMoney",date,bindelement);
			}
			// 取款总额
			if(data&&data.teamTakeMoney){
				this.stat("teamTakeMoney",data.teamTakeMoney);
			}else{
				this.getData("stat","teamTakeMoney","${ctx}/Pandect/ETakeMoney",date,bindelement);
			}
			// 团队余额
			if(data&&data.teamBalance){
				this.stat("teamBalance",data.teamBalance);
			}else{
				this.getData("stat","teamBalance","${ctx}/Pandect/EBalance",{},bindelement);
			}
			// 冲正统计
			if(data&&data.integralPlus){
				this.stat("integralPlus",data.integralPlus);
			}else{
				this.getData("stat","integralPlus","${ctx}/Pandect/EIntegralPlus",date,bindelement);
			}
			// 冲负统计
			if(data&&data.integralMinus){
				this.stat("integralMinus",data.integralMinus);
			}else{
				this.getData("stat","integralMinus","${ctx}/Pandect/EIntegralMinus",date,bindelement);
			}
			// 优惠额
			if(data&&data.discountAmount){
				this.stat("discountAmount",data.discountAmount);
			}else{
				this.getData("stat","discountAmount","${ctx}/Pandect/EDiscountAmount",date,bindelement);
			}
			// 洗码额
			if(data&&data.washingAmount){
				this.stat("washingAmount",data.washingAmount);
			}else{
				this.getData("stat","washingAmount","${ctx}/Pandect/EWashingAmount",date,bindelement);
			}
			...
			if(data&&data.teamPerformanceCanvas){
				...
				this.getData("circle","teamPerformanceCanvas","${ctx}/Pandect/EPerformance",date,bindelement);
			}
			
			if(data&&data.teamPerformanceBrandCanvas){
				this.circle("teamPerformanceBrandCanvas",data.teamPerformanceBrandCanvas)
			}else{
				this.getData("circle","teamPerformanceBrandCanvas","${ctx}/Pandect/EBrandPerformance",date,bindelement);
			}
			
			if(data&&data.teamPerUseractivityCanvas){
				this.polyline("teamPerUseractivityCanvas",data.teamPerUseractivityCanvas);
			}else{
				this.getData("polyline","teamPerUseractivityCanvas","${ctx}/Pandect/EUseractivity",{},bindelement);
			}
		}
	}
