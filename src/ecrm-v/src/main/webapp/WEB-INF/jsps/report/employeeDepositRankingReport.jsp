...
<div class="detail-section" style="text-align: center;">
    <div class="sdateinfo">
      ...
      <button id="btnSearch" type="submit"  class="button button-primary" onclick="depositRankingQuery()"><span class="icon-search icon-white"></span>搜 索 </button>
    </div>
    <div class="srepinfo">
	    ...
	    <div style="width: 240px; text-align: left; margin-top:10px;" class="tips tips-small tips-notice">
	        ...
	        <div class="tips-content">该报表将会统计某一段时间内，<br>
	        	代理直线存款最多的前10个用户，根据存款金额排倒序；<br><br>
  ...
  <script type="text/javascript">
   var chart;
   depositRankingQuery();
   srightBuiTip("#t2", "该报表将会统计某一段时间内，代理直线存款最多的前10个用户，根据存款金额排倒序；<br>"+
		   "可选操作条件【起始时间，结束时间】。");
    function depositRankingQuery(){
    	...
    	$.ajax({
    		url:"${ctx}/report/queryEmployeeDepositRankingReport",
    		type:"post",
    		...
    		success:function(data){
    			...
    			for(var i=0;i<data.length;i++){
  	  				//array.push({x : data[i].loginaccount,y : data[i].orderamount,attrs : {fill : color[i]},employeecode:data[i].employeecode});
  	  				if (i == 0){
	                   	...
	                   	itemclick: function (ev) {
		                    ...
		                    if(point.value > 0){
			  		            top.topManager.openPage({
				    		      	id : 'queryEmployeeDepositRankingDeatil',
				    		      	href : getRootPath()+'/report/employeeDepositRankingReportDetail?employeecode='+employeecode.get(point.xValue)+"&orderStartDate="+begin_date+"&orderEndDate="+end_date,
				    		      	title : '存款明细'
				    		     });
		                   	}
	                    }
