		...
		handler : function() {
			...
			if (selectItem.length == 0) {
				...
				BUI.Message.Confirm('是否确定发放？(当前已选择'+selectItem.length+'条)',function() {
					...
					$.ajax({
						type : "POST",
						url : "${ctx}/reportAgent/processDailySelect",
	...
	botton_arry.push({
		...
		handler : function() {
			...
			BUI.Message.Confirm('注意：：：：是否确定"全部"发放？(当前没有未发放的将会全部发放，请注意)',function() {
				...
				$.ajax({
					type : "POST",
					url : "${ctx}/reportAgent/processDailyAll",
					/* data : {
						"sign" : sign
					}, */
					dataType : "json",
...
if(export_excel){
	botton_arry.push({
	          ...
	          handler:function(){
	          	searchForm.action = "${ctx}/reportAgent/excelDailyData";
	          	searchForm.submit();
	          	searchForm.action = "${ctx}/reportAgent/reportDailyData";
	          }
	});
}
if(cancel_plan){
	botton_arry.push({
        ...
        handler : function(){
      	 openNewWindow('cancel_daily_agent','${ctx}/reportAgent/cancelDaily','取消支付计划');
        }
    });	
}
...
var Grid = BUI.Grid,
  ...
  columns = [
	...
	{ title: '洗码金额',width: '8%',elCls:'center',  sortable: true, summary : true, dataIndex: 'amount', renderer:function(value){
		if( value==null || value == "") {
			return 0;
		} else {
	...
	{ title: '洗码比率',width: '8%',elCls:'center',  sortable: true,  dataIndex: 'ratio'},
	{ title: '投注日期',width: '8%',elCls:'center',  sortable: false,  dataIndex: 'time', renderer : BUI.Grid.Format.dateRenderer},
	{ title: '汇总时间',width: '12%',elCls:'center',  sortable: false,  dataIndex: 'reporttime', renderer : BUI.Grid.Format.datetimeRenderer},
...
var store = new Store({
    url : '${ctx}/reportAgent/reportDailyData',
    autoLoad:false, 
    pageSize:15
  }),
  grid = new Grid.Grid({
