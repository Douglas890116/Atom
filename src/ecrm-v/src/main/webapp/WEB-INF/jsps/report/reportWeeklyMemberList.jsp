...
<ul style="border:5px; height:20px; line-height:20px; color:red; font-weight:bold;" class="breadcrumb">
	...
	每周一18：30分汇总上周一至上周日的未发放的会员洗码数据。（请在这个时间之前尽可能发放周日的洗码）
	</span>
	</li>
</ul>
...
var process_select = ${sessionScope.ERCM_USER_PSERSSION['MN00AW'].menustatus ==1 };
var process_all = ${sessionScope.ERCM_USER_PSERSSION['MN00AX'].menustatus ==1 };
var cancel_plan = ${sessionScope.ERCM_USER_PSERSSION['MN00EX'].menustatus ==1 };
	...
	botton_arry.push({
		...
		handler : function() {
			...
			if (selectItem.length == 0) {
				BUI.Message.Alert('请选择需要发放的数据', 'info');
			} else {
				BUI.Message.Confirm('是否确定发放？(当前已选择'+selectItem.length+'条)',function() {
					...
					$.ajax({
						type : "POST",
						url : "${ctx}/reportMember/processWeeklySelect",
						dataType : "json",
	...
	botton_arry.push({
		...
		handler : function() {
			...
			BUI.Message.Confirm('注意：：：：是否确定"全部"发放？(当前没有未发放的将会全部发放，请注意)',function() {
				...
				$.ajax({
					type : "POST",
					url : "${ctx}/reportMember/processWeeklyAll",
					/* data : {
						"sign" : sign
					}, */
					dataType : "json",
...
if(cancel_plan){
	botton_arry.push({
        ...
        handler : function(){
      	 openNewWindow('cancel_weekly_member','${ctx}/reportMember/cancelWeekly','取消支付计划');
        }
    });	
}
...
function processWeekCheck(reportcode,objx) {
	BUI.Message.Confirm('是否确定进行校验？可能会需要一点时间',function() {
		$.ajax({
			type : "POST",
			url : "${ctx}/reportMember/processWeekCheck",
...
function processWeekSupplement(reportcode,objx) {
	BUI.Message.Confirm('是否确定现在进行补发？可能会需要一点时间，补发前将会做二次校验',function() {
		$.ajax({
			type : "POST",
			url : "${ctx}/reportMember/processWeekSupplement",
			data : {"reportcode": reportcode},
			dataType : "json",
			success : function(data) {
...
function savePayMoney() {
	...
	BUI.Message.Confirm('是否确定操作吗？',function() {
		$.ajax({
			type : "POST",
			url : "${ctx}/reportMember/updateMoney",
...
var Grid = BUI.Grid,
  ...
  columns = [
	{ title: '用户名',width: '8%',elCls:'center',  sortable: false,  dataIndex: 'loginaccount'},
	{ title: '游戏平台',width: '6%',elCls:'center',  sortable: false,  dataIndex: 'gameplatform'},
	...
	{ title: '有效投注额',width: '8%',elCls:'center',  sortable: true, summary : true, dataIndex: 'bet', renderer:function(value){
		return value.toFixed(2);
	}},
	{ title: '应发放金额',width: '8%',elCls:'center',  sortable: true, summary : true, dataIndex: 'amount', renderer:function(value){
		return value.toFixed(2);
	}},
	{ title: '实际应发额', width: '8%',elCls:'center',  sortable: true, summary : true, dataIndex: 'actual', renderer:function(value){
	...
	{ title: '已发放',width: '8%',elCls:'center',  sortable: true, summary : true, dataIndex: 'subtract', renderer:function(value){
		if( value==null || value == "" || value == 0) {
	...
	{ title: '未发放',width: '8%',elCls:'center',  sortable: true, summary : true, renderer:function(value,obj){
		if(obj.status == 1) {
	...
	{ title: '状态',width: '6%',elCls:'center',  sortable: false,  dataIndex: 'status', renderer:function(value){
		if(value==1) {
	...
	{ title: '报表统计区间',width: '12%',elCls:'center',  sortable: false, renderer: function(value,obj){
		var str1=FormatDate(obj.starttime);
		var str2=FormatDate(obj.endtime);
		return str1+"至"+str2;
	}},
	{ title: '汇总时间',width: '12%',elCls:'center',  sortable: true,  dataIndex: 'reporttime', renderer : BUI.Grid.Format.datetimeRenderer},
	{ title: '支付/取消单号',width: '12%',elCls:'center',  sortable: false,  dataIndex: 'payno', renderer:function(value,obj){
		return value;
	}},
    { title: '操作',   width: '16%', elCls:'center', sortable: false, dataIndex: '',renderer:function(value,obj){
    	if(obj.paytype == "1") {
...
var store = new Store({
    url : '${ctx}/reportMember/reportWeeklyData',
    autoLoad:false, 
    pageSize:15,
    remoteSort: true,
    sortInfo : {
  ...
  grid = new Grid.Grid({
	...
	datepicker.set('dateMask','yyyy-mm-dd');
