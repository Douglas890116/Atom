...
function showMember(employeecode){
	top.topManager.openPage({
	  	id : 'member_detail',
	  	href : getRootPath()+'/report/userProfitReport?employeecode='+employeecode+"&employeeType=T003",
	 	title : '下级会员'
	});
}
function showAgent(employeecode){
	top.topManager.openPage({
	  	id : 'agent_detail',
	  	href : getRootPath()+'/report/userProfitReport?employeecode='+employeecode+"&employeeType=T004",
	 	title : '代理团队'
	});
}
var Grid = BUI.Grid,
  ...
  columns = [
	...
	{ title: '用户类型',width: '15%',elCls:'center',  sortable: false,  dataIndex: 'employeetypename',renderer:function(value){
		return value.replace("_nodown","");
	}},
	//{ title: '用户数量',width: '10%',elCls:'center',  sortable: false,  dataIndex: 'num'},
	{ title: '投注总额',width: '15%',elCls:'center',  sortable: false,  dataIndex: 'game_betting_amount',summary : true, renderer:function(value,obj){
		return "<span style='color:#29B529;font-size:medium;'>"+(value==null?"0.00":parseFloat(value).toFixed(2))+"</span>";
	}},
	{ title: '输赢金额',width: '15%',elCls:'center',  sortable: false,  dataIndex: 'lose_win_amount',summary : true, renderer:function(value){
		if((value+"").indexOf("-")<0){
			return "<span style='color:#29B529;font-size:medium;'>"+(value==null?"0.00":parseFloat(value).toFixed(2))+"</span>";
		}else{
			return "<span style='color:red;font-size:medium;'>"+(value==null?"0.00":parseFloat(value).toFixed(2))+"</span>";			
		}
	}},
	{ title: '洗码金额',width: '15%',elCls:'center',  sortable: false,  dataIndex: 'bonus',summary : true, renderer:function(value,obj){
		return "<span style='color:#29B529;font-size:medium;'>"+(value==null?"0.00":parseFloat(value).toFixed(2))+"</span>";
	}},
	{ title: '分红金额',   width: '10%', elCls:'center', sortable: false, dataIndex: 'dividend',summary : true, renderer:function(value,obj){
		return "<span style='color:#29B529;font-size:medium;'>"+(value==null?"0.00":parseFloat(value).toFixed(2))+"</span>";
	}},
	{ title: '占成金额',   width: '10%', elCls:'center', sortable: false, dataIndex: 'share',summary : true, renderer:function(value,obj){
		return "<span style='color:#29B529;font-size:medium;'>"+(value==null?"0.00":parseFloat(value).toFixed(2))+"</span>";
	}},
	{ title: '利润金额',   width: '10%', elCls:'center', sortable: false, dataIndex: 'profit_amount',summary : true, renderer:function(value,obj){
		return "<span style='color:#29B529;font-size:medium;'>"+(value==null?"0.00":parseFloat(value).toFixed(2))+"</span>";
	}}
  ];

var store = new Store({
    url : '${ctx}/report/queryUserProfit',
    autoLoad:false, 
    pageSize:15
  }),grid = new Grid.Grid({
    render:'#grid',
