...
<body>
    ...
    <table width="100%" cellpadding="4" cellspacing="0" class="toTab" border="0">
        <thead>
            ...
            <th class="toLabelTd">游戏代号</th>
            <th class="toLabelTd">项目类别</th>
        ...
        <tbody>
            <c:forEach var="item" items="${listData}" varStatus="i">
                <tr>
                    ...
                    <td class="toDataTd">'${item.mgColId}</td>
                    <td class="toDataTd">${item.loginaccount}</td>

                    <td class="toDataTd">${item.mgGameId}</td>
                    <td class="toDataTd">${item.mgTransType}</td>
                    <td class="toDataTd">${item.mgMgsGameId}</td>
                    
                    <td class="toDataTd"><fmt:formatNumber value="${item.mgAmount}" pattern="##.##" minFractionDigits="2"/></td>
                    <td class="toDataTd"><fmt:formatNumber value="${item.netMoney}" pattern="##.##" minFractionDigits="2"/></td>
                    <td class="toDataTd"><fmt:formatNumber value="${item.mgBalance}" pattern="##.##" minFractionDigits="2"/></td>
