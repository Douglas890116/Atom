    	...
    	<tbody>
    		<c:forEach var="item" items="${listData}" varStatus="i">
    			<tr>
    				...
    				<td class="toDataTd">${item.friendlygameinstanceid}</td>
    				<td class="toDataTd">${item.loginaccount}</td>
    				<td class="toDataTd">${item.username}</td>
    				<td class="toDataTd">${item.gamekeyname}</td>
    				...
    				<td class="toDataTd"><fmt:formatNumber value="${item.jackpotwin}" pattern="##.##" minFractionDigits="2"/></td>
    				<td class="toDataTd"><fmt:formatDate value="${item.dtstarted}" pattern="yyyy-MM-dd HH:mm:ss"/></td>    			
    				<td class="toDataTd"><fmt:formatDate value="${item.dtcompleted}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
    			</tr>
    		</c:forEach>
    	</tbody>
    </table>
