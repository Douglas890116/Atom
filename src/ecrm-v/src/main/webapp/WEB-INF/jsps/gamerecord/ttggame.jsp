  ...
  <input name="employeecode" type="hidden" value="${employeecode}"/>
    ...
    <div style="position: relative;display: inline-block;">
      <div style="float: left;margin-right: 96px;">
           ...
           <div class="control-group span7">
              <label class="control-label">游戏名称：</label>
                <input name="game"  type="text" class="control-text" placeholder="游戏名称"/>
            </div>
            <div class="control-group span7">
  ...
  <div id="grid"></div>
   ...
   if(export_excel){
    botton_arry.push({
              ...
              handler:function(){
                searchForm.action = "${ctx}/TtgGame/exportExcel";
                searchForm.submit();
                searchForm.action = "${ctx}/TtgGame/data";
              }
    });
   }
   $(function(){
	   ...
	   if(!begintime.val()&&!endtime.val()){
        ...
        var Grid = BUI.Grid,
          ...
          columns = [
			{ title: '投注单号',   width: '11%',  sortable: true,dataIndex:'handid'},
			{ title: '用户账号',width: '12%',  sortable: false,dataIndex:'loginaccount'},
			{ title: '交易类型',   width: '15%',  sortable: false,dataIndex:'transactiontype'},
			{ title: '游戏类型',   width: '15%',  sortable: false,dataIndex:'game'},
			{ title: '投注金额',   width: '10%',  sortable: false,summary : true, dataIndex:'amount', renderer:function(value){
			...
			{ title: '结算时间',   width: '20%',  sortable: false,dataIndex:'resolvetime',renderer:BUI.Grid.Format.datetimeRenderer}
          ];

        var store = new Store({
            url : '${ctx}/TtgGame/data',
            autoLoad : false,
            pageSize:15,
            remoteSort: true,
            sortInfo : {
