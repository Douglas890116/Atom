...
<body>
    ...
    <table width="100%" cellpadding="4" cellspacing="0" class="toTab" border="0">
        <thead>
            ...
            <th class="toLabelTd">平台编号</th>
            <th class="toLabelTd">平台类型</th>
            
            <th class="toLabelTd">游戏类型</th>
            <th class="toLabelTd">游戏玩法</th>
        ...
        <tbody>
            <c:forEach var="item" items="${listData}" varStatus="i">
                <tr>
                    ...
                    <td class="toDataTd">${item.platformId}</td>
                    <td class="toDataTd">${item.platformType}</td>
                    
                    <td class="toDataTd">${item.gameType}</td>
                    <td class="toDataTd">${item.playType}</td>
                    <td class="toDataTd">${item.tableCode}</td>
                    <td class="toDataTd">${item.gameCode}</td>
                    
                    <td class="toDataTd"><fmt:formatNumber value="${item.betAmount}" pattern="##.##" minFractionDigits="2"/></td>
                    <td class="toDataTd"><fmt:formatNumber value="${item.validBetAmount}" pattern="##.##" minFractionDigits="2"/></td>
                    <td class="toDataTd"><fmt:formatNumber value="${item.netAmount}" pattern="##.##" minFractionDigits="2"/></td>
