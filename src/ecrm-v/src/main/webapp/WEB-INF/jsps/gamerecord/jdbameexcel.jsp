        ...
        <tbody>
            <c:forEach var="item" items="${listData}" varStatus="i">
                <tr>
                    ...
                    <td class="toDataTd">${item.seqNo}</td>
                    <td class="toDataTd">${item.loginaccount}</td>
                    <td class="toDataTd">
                      <c:if test="${item.gType == 0}">老虎機</c:if>
                      <c:if test="${item.gType == 7}">捕魚機</c:if>
                      <c:if test="${item.gType == 9}">水果機</c:if>
                    </td>
                    
                    <td class="toDataTd"><fmt:formatNumber value="${item.bet}" pattern="##.##" minFractionDigits="2"/></td>
                    <td class="toDataTd"><fmt:formatNumber value="${item.total}" pattern="##.##" minFractionDigits="2"/></td>
                    <td class="toDataTd">${item.clientType}</td>
                    <td class="toDataTd"><fmt:formatDate value="${item.gameDate}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
                </tr>
            </c:forEach>
        </tbody>
    </table>
