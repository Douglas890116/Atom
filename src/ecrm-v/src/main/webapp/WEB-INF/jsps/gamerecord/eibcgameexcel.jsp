...
<body>
    ...
    <table width="100%" cellpadding="4" cellspacing="0" class="toTab" border="0">
		<thead>
			...
			<th class="toLabelTd">登录账号</th> 
			<th class="toLabelTd">投注单号</th> 
			<th class="toLabelTd">联赛名称</th>
			<th class="toLabelTd">客队名称</th>
			<th class="toLabelTd">主场球队名称</th>
			<th class="toLabelTd">运动类型</th>
			<th class="toLabelTd">投注类型</th>
			
			<th class="toLabelTd">投注站团队</th>
			<th class="toLabelTd">赔率</th>
			<th class="toLabelTd">客队得分</th>
			<th class="toLabelTd">主场球队得分</th>
    	...
    	<tbody>
    		<c:forEach var="item" items="${listData}" varStatus="i">
    			<tr>
    				...
    				<td class="toDataTd">${item.awayidname}</td>
    				<td class="toDataTd">${item.homeidname}</td>
    				<td class="toDataTd">${item.sporttype}</td>
    				<td class="toDataTd">${item.bettype}</td>
    				<td class="toDataTd">${item.betteam}</td>
    				<td class="toDataTd">${item.odds}</td>
    				<td class="toDataTd">${item.awayscore}</td>
    				<td class="toDataTd">${item.homescore}</td>
    				
    				<td class="toDataTd">${item.ticketstatus}</td>
