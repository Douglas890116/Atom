      ...
      <div style="float: left;margin-right: 96px;">
           ...
           <div class="control-group span7">
              <label class="control-label">投注单号：</label>
                <input name="id"  type="text" data-tip='{"text":"投注单号"}' value="${param.id }" class="control-text" placeholder="投注单号"/>
            </div>
           ...
           <div class="control-group span7 toggle-display">
              <label class="control-label">有效下注金额：</label>
                <input name="validStake"  type="text" data-tip='{"text":"有效下注金额"}' value="${param.validStake }" class="control-text" placeholder="有效下注金额"/>
            </div>
      </div>
  ...
  <div id="grid"></div>
   ...
   if(export_excel){
    botton_arry.push({
              ...
              handler:function(){
                searchForm.action = "${ctx}/ZjGame/exportExcel";
                searchForm.submit();
                searchForm.action = "${ctx}/ZjGame/data";
              }
    });
   }
   $(function(){
	   ...
	   if(!begintime.val()&&!endtime.val()){
        ...
        var Grid = BUI.Grid,
          ...
          columns = [
			{ title: '投注单号',   width: '11%',  sortable: true,dataIndex:'id'},
			{ title: '用户账号',width: '12%',  sortable: false,dataIndex:'loginaccount'},
			{ title: '游戏ID',   width: '12%',  sortable: false,dataIndex:'gametype', renderer:function(value){
				switch (value) {
				case 'BACCARAT':
					return '百家乐';
				case 'SICBO':
			...
			{ title: '桌台ID',   width: '15%',  sortable: false,dataIndex:'tableinfoid'},
			{ title: '下注金额',   width: '10%',  sortable: false,summary : true, dataIndex:'stakeamount', renderer:function(value){
				return value.toFixed(2);
			}},
			{ title: '有效下注金额',   width: '10%',  sortable: false,summary : true, dataIndex:'validstake', renderer:function(value){
				return value.toFixed(2);
			}},
			{ title: '下注结果',   width: '10%',  sortable: false,summary : true, dataIndex:'winloss', renderer:function(value){
				return value.toFixed(2);
			}},
			{ title: '下注后余额',   width: '10%',  sortable: false,summary : true, dataIndex:'balanceafter', renderer:function(value){
				return value.toFixed(2);
			}},
