    ...
    <div style="position: relative;display: inline-block;">
      <div style="float: left;margin-right: 96px;">
           <div class="control-group span13">
                <label class="control-label">${isEnglish?'Settlement date':'结算日期'}：</label>
            ...
            <div class="control-group span6">
              <label class="control-label">${isEnglish?'Parent Account':'上级账号'}：</label>
  ...
  <div id="grid"></div>
   ...
   if(export_excel){
   	botton_arry.push({
   	          ...
   	          handler:function(){
   	          	searchForm.action = "${ctx}/ggpgame/exportExcel";
   	          	searchForm.submit();
   	          	searchForm.action = "${ctx}/ggpgame/data";
   	          }
   	});
   }
   ...
   $(function(){
	   ...
	   if(!begintime.val()&&!endtime.val()){
        ...
        var Grid = BUI.Grid,
          ...
          columns = [
			{ title: '${isEnglish?"Account":"会员账号"}',   width: '15%',  sortable: false,dataIndex:'loginaccount'},
			{ title: '${isEnglish?"date":"结算日期"}',   width: '10%',  sortable: false,dataIndex:'gamedate'},
			{ title: '${isEnglish?"Rake(RMB)":"抽水(人民币)"}',   width: '15%',  sortable: false,summary : true, dataIndex:'betmoney', renderer:function(value){
				return "<font color='red'>"+parseFloat(value).toFixed(2)+"</font>";
			}},
			{ title: '${isEnglish?"Rake(USD)":"抽水(美元)"}',   width: '10%',  sortable: false,summary : false, dataIndex:'ngr', renderer:function(value){
				return "<font color='red'>"+parseFloat(value).toFixed(2)+"</font>";
			}},
			{ title: '${isEnglish?"Record Time":"记录时间"}',   width: '20%',  sortable: false,dataIndex:'createtime',renderer:BUI.Grid.Format.datetimeRenderer}
          ];

        var store = new Store({
            url : '${ctx}/ggpgame/data',
            autoLoad : false,
            pageSize:15,
            remoteSort: true,
            sortInfo : {
