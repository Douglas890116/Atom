...
<body>
    ...
    <table width="100%" cellpadding="4" cellspacing="0" class="toTab" border="0">
        <thead>
            ...
            <th class="toLabelTd">平台类型</th>
            <th class="toLabelTd">游戏类型</th>
            <th class="toLabelTd">游戏代码</th>
        ...
        <tbody>
            <c:forEach var="item" items="${listData}" varStatus="i">
                <tr>
                    ...
                    <td class="toDataTd">${item.roundid}</td>
                    <td class="toDataTd">${item.loginaccount}</td>
                    <td class="toDataTd">${item.userid}</td>
                    
                    
                    <td class="toDataTd">${item.platformtype}</td>
                    <td class="toDataTd">${item.gameprovider}</td>
                    <td class="toDataTd">${item.gameid}</td>
                    <td class="toDataTd">${item.gamename}</td>
                    ...
                    <td class="toDataTd"><fmt:formatNumber value="${item.postbal}" pattern="##.##" minFractionDigits="2"/></td>
                    ...
                    <td class="toDataTd"><fmt:formatDate value="${item.timestamp}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
                </tr>
            </c:forEach>
        </tbody>
    </table>
