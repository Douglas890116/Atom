...
<body>
    ...
    <table width="100%" cellpadding="4" cellspacing="0" class="toTab" border="0">
        <thead>
            ...
            <th class="toLabelTd">下注来源</th>
            <th class="toLabelTd">下注类型</th>
            <th class="toLabelTd">结算</th>
        ...
        <tbody>
            <c:forEach var="item" items="${listData}" varStatus="i">
                <tr>
                    ...
                    <td class="toDataTd">'${item.detail}</td>
                    <td class="toDataTd">'${item.gameid}</td>
                    <td class="toDataTd">${item.hostid}</td>
                    <td class="toDataTd">${item.round}</td>
                    <td class="toDataTd">${item.sets}</td>
                    
                    <td class="toDataTd"><fmt:formatNumber value="${item.betamount}" pattern="##.##" minFractionDigits="2"/></td>
                    <td class="toDataTd">${item.state}</td>
                    <td class="toDataTd">${item.betsource}</td>
                    <td class="toDataTd">${item.bettype}</td>
                    <td class="toDataTd">${item.resultamount}</td>
                    <td class="toDataTd"><fmt:formatDate value="${item.payouttime}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
                </tr>
            </c:forEach>
        </tbody>
    </table>
