           ...
           <div class="control-group span14 ">
            ...
            <div class="control-group span7">
              <label class="control-label">游戏代号：</label>
                <input name="gamekeyname"  type="text" data-tip='{"text":"游戏代号"}' class="control-text" placeholder="游戏代号"/>
            </div>
            <div class="control-group span7">
  ...
  <div id="grid"></div>
   ...
   if(export_excel){
   	botton_arry.push({
   	          ...
   	          handler:function(){
   	          	searchForm.action = "${ctx}/habgame/exportExcel";
   	          	searchForm.submit();
   	          	searchForm.action = "${ctx}/habgame/data";
   	          }
   	});
   }
   ...
   $(function(){
	   ...
	   if(!begintime.val()&&!endtime.val()){
        ...
        var Grid = BUI.Grid,
          ...
          columns = [
			{ title: '投注单号',   width: '11%',  sortable: true,dataIndex:'friendlygameinstanceid'},
			{ title: '游戏代号',   width: '12%',  sortable: false,dataIndex:'gamekeyname'},
			{ title: '游戏账号',   width: '15%',  sortable: false,dataIndex:'username'},
			{ title: '投注金额',   width: '10%',  sortable: false,summary : true, dataIndex:'betmoney', renderer:function(value){
				return parseFloat(value).toFixed(2);
			}},
			{ title: '奖池金额',   width: '10%',  sortable: false,summary : false, dataIndex:'jackpotwin', renderer:function(value){
				return parseFloat(value).toFixed(2);
			}},
			{ title: '输赢金额',   width: '10%',  sortable: false,summary : true, dataIndex:'netmoney', renderer:function(value){
				return parseFloat(value).toFixed(2);
			}},
			{ title: '投注时间',   width: '20%',  sortable: false,dataIndex:'dtstarted',renderer:BUI.Grid.Format.datetimeRenderer},
			{ title: '结算时间',   width: '20%',  sortable: false,dataIndex:'dtcompleted',renderer:BUI.Grid.Format.datetimeRenderer}
			
          ];

        var store = new Store({
            url : '${ctx}/habgame/data',
            autoLoad : false,
            pageSize:15,
            remoteSort: true,
            sortInfo : {
