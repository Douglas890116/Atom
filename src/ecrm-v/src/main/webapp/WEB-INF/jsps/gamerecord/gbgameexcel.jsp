...
<body>
	<table width="100%" border="1">
 		<tr>
  			<td colspan="10" bgcolor="#FAFAD2" align="center"><h3>${title}</h3></td>
  		</tr>
  		<tr>
  			<td colspan="10" bgcolor="#FAFAD2">下单设备说明 : 1.行动装置下单：M 1-1.ios手机：MI1 1-2.ios平板：MI2 1-3.Android手机：MA1 1-4.Android平板：MA2 2.计算机下单：P</td>
  		</tr>
  	</table>
    <table width="100%" cellpadding="4" cellspacing="0" class="toTab" border="0">
		<thead>
			...
			<th class="toLabelTd">返还类型</th>
			<th class="toLabelTd">返还金额</th>
    	...
    	<tbody>
    		<c:forEach var="item" items="${listData}" varStatus="i">
    			<tr>
    				...
    				<td class="toDataTd">${item.settleid}</td>
    				<td class="toDataTd">${item.loginaccount}</td>
    				<td class="toDataTd">${item.memberid}</td>
    				...
    				<td class="toDataTd">
						<c:if test="${item.bettype != null && item.bettype eq 1}">獨贏</c:if>
						<c:if test="${item.bettype != null && item.bettype eq 1}">串關(M*N)</c:if>
						<c:if test="${item.bettype != null && item.bettype eq 1}">追號(連下M期)</c:if>
					</td>
    				<td class="toDataTd"><fmt:formatNumber value="${item.refundbetamt}" pattern="##.##" minFractionDigits="2"/></td>
