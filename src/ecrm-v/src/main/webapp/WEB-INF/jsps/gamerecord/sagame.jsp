      ...
      <div style="float: left;margin-right: 96px;">
           ...
           <div class="control-group span7">
              ...
              <select name="gametype">
                  ...
                  <option value="dtx">龙虎</option>
                  <option value="sicbo">骰宝</option>
                  <option value="ftan">番摊</option>
                  <option value="rot">轮盘</option>
                  <option value="slot">电子游艺</option>
                  <option value="lottery">48 彩/48彩其他玩法</option>
           ...
           <div class="control-group span7 toggle-display">
              <label class="control-label">有效下注金额：</label>
                <input name="betamount"  type="text" data-tip='{"text":"有效下注金额"}' class="control-text" placeholder="有效下注金额"/>
            </div>
      </div>
  ...
  <div id="grid"></div>
   ...
   if(export_excel){
    botton_arry.push({
              ...
              handler:function(){
                searchForm.action = "${ctx}/SaGame/exportExcel";
                searchForm.submit();
                searchForm.action = "${ctx}/SaGame/data";
              }
    });
   }
   $(function(){
	   ...
	   if(!begintime.val()&&!endtime.val()){
        ...
        var Grid = BUI.Grid,
          ...
          columns = [
			{ title: '投注单号',   width: '11%',  sortable: false,dataIndex:'betid'},
			{ title: '用户账号',width: '12%',  sortable: false,dataIndex:'loginaccount'},
			{ title: '游戏名称',   width: '12%',  sortable: false,dataIndex:'detail'},
			{ title: '游戏类型',   width: '12%',  sortable: false,dataIndex:'gametype'},
			{ title: '桌台ID',   width: '15%',  sortable: false,dataIndex:'hostid'},
			{ title: '下注金额',   width: '10%',  sortable: false,summary : true, dataIndex:'betamount', renderer:function(value){
				return value.toFixed(2);
			}},
			{ title: '输赢金额',   width: '10%',  sortable: false,summary : true, dataIndex:'resultamount', renderer:function(value){
				return value.toFixed(2);
			}},
			{ title: '有效投注额',   width: '10%',  sortable: false,summary : true, dataIndex:'validbet', renderer:function(value){
				return value.toFixed(2);
			}},
			/* { title: '下注结果',   width: '10%',  sortable: false, dataIndex:'state'},
			{ title: '下注来源',   width: '10%',  sortable: false,dataIndex:'betsource'}, */
			{ title: '投注时间',   width: '20%',  sortable: false,dataIndex:'payouttime',renderer:BUI.Grid.Format.datetimeRenderer}/* ,
