        ...
        function permissionValidate(){
        	...
        	if(add_brand){
        		array.push({
                  ...
                  text:'<span class="icon-file icon-white"></span>管理游戏',
                  handler : function(){
                	 openNewWindow('create_brand','${ctx}/enterprisesolt/golist','管理游戏');
                  }
              });
        	}
        	return array;
        ...
        var Grid = BUI.Grid,
          ...
          columns = [
            ...
            { title: '操作',width:'16%',sortable: false,renderer:function(value,obj){
				...
				if(obj.status == "1") {
					html += '<button type="button" onclick="test1(\''+ obj.lsh+'\', 1);" class="button  button-info botton-margin">禁用</button>';
				} else {
					html += '<button type="button" onclick="test1(\''+ obj.lsh+'\', 0);" class="button  button-info botton-margin">启用</button>';
				}
