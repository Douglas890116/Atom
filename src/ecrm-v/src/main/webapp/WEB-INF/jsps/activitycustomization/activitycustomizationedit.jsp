...
<div class="demo-content" style="margin: 27px;">
    ...
    <div class="row">
      <div>
        ...
        <form aria-pressed="false" aria-disabled="false"  name="example" id="objForm" action="${ctx}/ActivityTemplate/Save" class="form-horizontal bui-form bui-form-field-container" method="post" >
            <input type="hidden" name="ecactivitycode" value="${activity.ecactivitycode }">
            <div class="control-group">
              ...
              <div class="controls">
                  ...
                  <c:forEach items="${enterprise }" var="ent">
                  	  <c:choose>
                  		<c:when test="${ent.enterprisecode==activity.enterprisecode }">
                  			<option value="${ent.enterprisecode }" selected="selected">${ent.enterprisename}</option>
                  		</c:when>
                  		<c:otherwise>
            ...
            <div class="control-group">
              <label class="control-label" style="margin-right: 12px;">活动名称：</label>
                <div class="controls">
                  <input name="activityname" value="${activity.activityname }" class="input-small control-text" style="width:370px;height:18px;" type="text" data-rules="{required:true}" data-tip="{text:'请输入模板名称'}"/>
                </div>
              ...
              <div class="controls">
                  ...
                  <c:forEach items="${template }" var="tm">
                  	  <c:choose>
                  	  	<c:when test="${tm.activitycode == activity.activitytemplatecode}">
                  	  		<option value="${tm.activitycode }" selected="selected">${tm.name }</option>
                  	  	</c:when>
                  	  	<c:otherwise>
            ...
            <div class="control-group">
              ...
              <div class="controls">
                  ...
                  <option value="">--请选择--</option>
                  <c:set var="unshare" value=",${activity.unshare }," ></c:set>
                  <c:forEach items="${activityList }" var="al">
                  	  <c:set var="ecactivitycode" value=",${al.ecactivitycode }," ></c:set>
                  	  <c:choose>
                  	  	<c:when test="${unshare.indexOf(ecactivitycode)>-1}">
                  	  		<option value="${al.ecactivitycode }" selected="selected">${al.activityname }</option>
                  	  	</c:when>
                  	  	<c:otherwise>
                  	  		<option value="${al.ecactivitycode }">${al.activityname }</option>
                  	  	</c:otherwise>
                  	  </c:choose>
                  </c:forEach>
                </select>
             ...
             <div class="control-group">
              <label class="control-label" style="margin-right: 12px;">活动PC端图片：</label>
                <div class="controls">
                	<input type="file" id="fileId" name="file" style="width:360px;" dir="rtl" />
                    ...
                    <input type="hidden"  name="activityimage" data-rules="{required:true}" value="${activity.activityimage }"/>
                    <span><img id="imgId" src="${activity.activityimage}" style="width: 220px;height: 45px;border: 1px solid #F6F6F6;margin-left: 150px;"></span>格式：812*144
                </div>
            </div>
             <div class="control-group">
              <label class="control-label" style="margin-right: 12px;">活动移动端图片：</label>
                <div class="controls">
                    <input type="file" id="fileH5" name="file" style="width:360px;" dir="rtl" />
                    ...
                    <input type="hidden" name="activityimagehfive" data-rules="{required:true}" value="${activity.activityimagehfive}"/>
                    <span><img id="h5iImgId" src="${activity.activityimagehfive}" style="width: 220px;height: 45px;border: 1px solid #F6F6F6;margin-left: 150px;"></span>格式：812*144
                </div>
            </div>
            <div class="control-group">
              <label class="control-label" style="margin-right: 12px;">PC活动描述（依据背景）：</label>
                <div class="controls">
                	<textarea name="activitycontent" style="width: 800px;height:400px; " class="bui-form-field">${activity.activitycontent }</textarea>
                </div>
            </div>
            ...
            <div class="control-group">
              <label class="control-label" style="margin-right: 12px;">H5活动描述（依据背景）：</label>
                <div class="controls">
                	<textarea name="activitycontenth5" style="width: 800px;height:400px; " class="bui-form-field">${activity.activitycontenth5 }</textarea>
                </div>
            </div>
            ...
            <div class="control-group">
              <label class="control-label" style="margin-right: 12px;">排序：</label>
                <div class="controls">
                	<input name="ord" value="${activity.ord }" class="input-small control-text" style="width:370px;height:18px;" type="text" data-rules="{required:true}" data-tip="{text:'请输入排序'}"/>
                </div>
            </div>
    ...
    $(function () {
    	...
    	var form = new BUI.Form.HForm({
   		...
   		bindClick("J_Form_Submit","${ctx}/EACustomization/Update",form);
