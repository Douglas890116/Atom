      ...
      <div>
        ...
        <hr>
            <form:form method="post" id="objForm" class="form-horizontal">
             <input type="hidden" name="sign" value="${paymentBank.sign}" />
              <div class="control-group">
                <label class="control-label">银行名称:</label>
                <div class="controls">
                    <select name="bankcode" data-rules="{required:true}">
                      	<c:forEach items="${paymentBankList}" var="brand">
							<option value="${brand.bankcode }" ${brand.bankcode==paymentBank.bankcode?"selected='selected'":"" } >${brand.bankname}</option>
						</c:forEach>
                    </select>
                </div>
              </div>
              <div class="control-group">
                ...
                <div class="controls">
                    <select name="thirdpartypaymenttypecode" data-rules="{required:true}">
                        <c:forEach items="${paymentTypeList}" var="brand">
							<option value="${brand.thirdpartypaymenttypecode }" ${brand.thirdpartypaymenttypecode==paymentBank.thirdpartypaymenttypecode?"selected='selected'":"" } >${brand.thirdpartypaymenttypename}</option>
						</c:forEach>
                    </select>
                </div>
              </div>
              <div class="control-group">
                ...
                <div class="controls">
                    <input class="input-normal control-text" name="paymenttypebankcode" value="${paymentBank.paymenttypebankcode}" type="text" data-rules="{required:true}" style="width:142px;height:18px;"/>
                </div>
              </div>
               ...
               <div class="control-group">
                ...
                <div class="controls">
                      <label class="radio" for="enable1"><input type="radio" id="enable1" name="enable" value="1" style="width:16px;"  ${paymentBank.enable==1?"checked='checked'":""} />启用</label>&nbsp;&nbsp;&nbsp;
                      <label class="radio" for="enable2"><input type="radio" id="enable2" name="enable" value="2" style="width:16px;"  ${paymentBank.enable==2?"checked='checked'":""} />禁用</label>
...
function validateInput(form){
	...
	if(isOk){
		...
		bindClick("J_Form_Submit","${ctx}/thirdpartyPaymentBank/updateSave",form);
	}
}
</script>
