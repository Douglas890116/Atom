...
@Controller
@RequestMapping("/QwpGame")
public class BettingQwpController extends BaseController {

	private static LoggerManager log = LoggerManager.getLogger(BettingQwpController.class.getName(), OutputManager.LOG_BETTING_QWP);

	@Resource(name = "bettingQwpServiceImpl")
	private BettingGameService<BettingQwp> bettingGameService;
	...
	public Map<String, Object> data(HttpServletRequest request, HttpServletResponse response, HttpSession session) {
		try {
			...
			List<BettingQwp> list = bettingGameService.takeRecord(object);
			...
			summary.put("turnScore", StringUtil.getDouble(result.get("betmoney")));
			summary.put("score", StringUtil.getDouble(result.get("netmoney")));
			summary.put("revenue", StringUtil.getDouble(result.get("revenue")));
			data.put("summary", summary);

			return data;
		} catch (Exception e) {
			log.Error("获取去玩棋牌游戏数据失败!", e);
			return null;
		}
	}
	...
	public String exportExcel(HttpServletRequest request, HttpSession session, Model model) {
		try {
			...
			model.addAttribute("title", "去玩棋牌游戏报表数据");
		} catch (Exception e) {
			log.Error(e.getMessage(), e);
		}
		return "/gamerecord/qwpgameexcel";
	}
