...
import com.maven.util.GGUtils;
import com.maven.utils.StringUtil;

@Controller
@RequestMapping("/gggame")
public class BettingGgController extends BaseController {
	...
	private BettingGameService<BettingGg> bettingGameService;
	...
	public Map<String, Object> data(HttpServletRequest request, HttpServletResponse response, HttpSession session) {
		try {
			...
			List<BettingGg> list = bettingGameService.takeRecord(object);
			for (BettingGg bettingGg : list) {
				bettingGg.setGameid(GGUtils.__GameType.get(bettingGg.getGameid()) + "(" + bettingGg.getGameid() + ")");
			}
	...
	public String exportExcel(HttpServletRequest request, HttpSession session, Model model) {
		try {
			...
			model.addAttribute("title", "GG小游戏报表数据");
		} catch (Exception e) {
			log.Error(e.getMessage(), e);
		}
		return "/gamerecord/gggameexcel";
	}
