...
public class BettingHqPngController extends BaseController{
	...
	private BettingGameService<BettingHqPng> bettingGameService;
	...
	public Map<String,Object> data(HttpServletRequest request , HttpServletResponse response , HttpSession session){
		try {
			...
			List<BettingHqPng> list = bettingGameService.takeRecord(object);
...
//			return super.formatPagaMap(list, count);
    ...
    public String exportExcel(HttpServletRequest request, HttpSession session, Model model){
        try {
            ...
            model.addAttribute("title", "PNG游戏报表数据");
        } catch (Exception e) {
            log.Error(e.getMessage(), e);
        }
        return "/gamerecord/pnggameexcel";
    }
