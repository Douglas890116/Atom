...
public class BettingYoplayController extends BaseController{
	
	private static LoggerManager log = LoggerManager.getLogger(BettingYoplayController.class.getName(), OutputManager.LOG_BETTINGHQXCP);
	
	@Resource(name="bettingYoplayServiceImpl")
	private BettingGameService<BettingYoplay> bettingGameService;
	@Resource
	...
	public Map<String,Object> data(HttpServletRequest request , HttpServletResponse response , HttpSession session){
		try {
			...
			List<BettingYoplay> list = bettingGameService.takeRecord(object);
    ...
    public String exportExcel(HttpServletRequest request, HttpSession session, Model model){
        try {
            ...
            model.addAttribute("title", "YoPLAY游戏报表数据");
        } catch (Exception e) {
            log.Error(e.getMessage(), e);
        }
        return "/gamerecord/yoplaygameexcel";
    }
