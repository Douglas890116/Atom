...
public class BankCheckUtil {
	...
	public static void main(String[] args) {
		System.out.println(getNameOfBank("6230943570000185206"));
		
		for (int i = 0; i < bankBin.length; i++) {
			System.out.println(i+"="+bankBin[i]+"="+bankName[i]);
		}
	}
	 ...
	 * @return
	 */
	public static String checkBankCard(String idCard) {
		Map<String,BanksCardBin> data = SystemCache.getInstance().getBankscardbinAll();
		
		//根据卡BIN算法，校验前6位数和前8位数
		String cardbin_6 = idCard.substring(0, 6);
		String cardbin_8 = idCard.substring(0, 8);
		
		if( !data.containsKey(cardbin_6)) {
			if( !data.containsKey(cardbin_8)) {
				return null;
			} else {
				return data.get(cardbin_8).getBankname();
			}
		} else {
			return data.get(cardbin_6).getBankname();
		}
	}
...
//		return "无法识别银行卡所属银行名称";
	...
	private final static String[] bankBin = { "621098", "622150", "622151", "622181", "622188", "955100", "621095",
			...
			"628267", "621392", "621481", "621310", "621396", "623251", "628351", "623094"};
	...
	private static final String[] bankName = { "邮储银行·绿卡通", "邮储银行·绿卡银联标准卡", "邮储银行·绿卡银联标准卡", "邮储银行·绿卡专用卡", "邮储银行·绿卡银联标准卡",
			...
			"东营莱商村镇银行·绿洲卡", "建设银行·单位结算卡", "玉溪市商业银行·红塔卡" , "建设银行 - 银联储蓄卡"};
