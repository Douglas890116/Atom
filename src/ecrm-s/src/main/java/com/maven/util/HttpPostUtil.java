...
public class HttpPostUtil {
	...
	public static String doPostSubmitMap(String httpUrl,SortedMap<Object,Object> params) {
        try {
	...
	public static String doPostSubmitMap(String httpUrl,Map<String, String> params) {
        try {
            ...
            if(httpResponse.getStatusLine().getStatusCode() == 200)  {
            	...
            	return "远程连接返回错误码："+httpResponse.getStatusLine().getStatusCode()+"";
            }
		} catch (Exception e) {
			e.printStackTrace();
			return -1+"";
	...
	public static String doPostSubmitMap2(String httpUrl,Map<String, String> params) {
        try {
        	HttpPost httpRequst = new HttpPost(httpUrl);//创建HttpPost对象  
            //填入各个表单域的�?
        	List paramsxx = new ArrayList<BasicNameValuePair>(); 
	...
	public static String doPostSubmitMap3(String httpUrl,Map<String, String> params) {
        try {
        	...
        	UrlEncodedFormEntity encodedFormEntity = new UrlEncodedFormEntity(paramsxx,"UTF-8");
    		httpRequst.setEntity(encodedFormEntity);
...
//            //将表单的值放入postMethod�?
            ...
            HttpClient httpClient = new HttpClient();
            //连接超时
            httpClient.getHttpConnectionManager().getParams().setConnectionTimeout(10 * 1000); //毫秒
            //读取超时
            httpClient.getHttpConnectionManager().getParams().setSoTimeout(10 * 1000);//毫秒
            int result = httpClient.executeMethod(postMethod);
            postMethod.releaseConnection();//释放连接
            if(true) {
