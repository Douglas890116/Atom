...
public class TAGGame {	
	 ...
	 * @return 数据列表
	 */
	public static List<Map<String, Object>> getTAGData(String agent, String dir, String max, FtpUtils ftp) {
		try {
			...
			if(size1 > 0){
				...
				list.addAll(getTAGData(agent,ftp,ls1));
			}
			//第2步拉取正常数据
			...
			int size2 = ls2.size();
			if(size2 > 0){			
				list.addAll(getTAGData(agent,ftp,ls2));
			}
			return list;
		} catch (Exception ex) {
			ex.printStackTrace();
		...
		try {
			...
			if(size > 0){
				...
				for(int i = 0; i < size; i++){
					String filePath = ls.get(i);
					String str = ftp.readFile(filePath);
	...
	private static String getMaxDate(String max){
		...
		if(max==null){//如果获取的最大日期为空，刚设置成当前日期减少12个小时			
			...
			// 美东时间
			max = sdf.format(sysCal.getTime())+"0000.xml";
		}
		return max;
	}
...
//		System.out.println(list.size());
		FtpUtils ftp = new FtpUtils("xml.agingames.com",21,"P36.tcbet88yule","9&!UhbiFWo","/");
		String max = "201611160000.xml";
		List<Map<String, Object>> list = getTAGData("xml.agingames.com","XIN",max,ftp);
		System.out.println(list.size());
