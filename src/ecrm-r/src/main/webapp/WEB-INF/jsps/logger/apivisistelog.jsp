    ...
    <div>
    	<c:choose>
   			<c:when test="${APILOGSWITH}">
				<a href="${ctx }/SystemVisite/ApiSwith?swith=off" class="logger-off" ></a> -- ${requestScope.MENUS.menuname}开关  
			</c:when>				
   			<c:otherwise>
   				<a href="${ctx }/SystemVisite/ApiSwith?swith=on" class="logger-on" ></a> -- ${requestScope.MENUS.menuname}开关  
   			</c:otherwise>
   		</c:choose>
    </div>
    ...
    <c:forEach items="${APILOG}" var="LOG">
    	<tr>
    		...
    		<td>
    			<c:choose>
    			<c:when test="${LOG.opencyclestat }">
	    			<a href="${ctx }/SystemVisite/ApiCycleSwith?url=${LOG.visiteURL }&swith=off">
    			...
    			<c:otherwise>
	    			<a href="${ctx }/SystemVisite/ApiCycleSwith?url=${LOG.visiteURL }&swith=on">
    ...
    <script type="text/javascript">
    	$(function () {
    		...
    		$(".statcyclesetting").blur(function(){
    			...
    			window.location.href="${ctx }/SystemVisite/UpdateApiStatCycle?url="+url+"&cycle="+$(this).val();
    		});
    		$(".viewSystemLog").click(function(){
    			...
    			if(top.topManager){
        			top.topManager.openPage({
        			    ...
        			    href :"${ctx }/SystemVisite/ApiCycleVisiteLog?url="+url,
        			    title : "API分时统计",
        			  }); 
        		}else{
        			window.open("${ctx }/SystemVisite/ApiCycleVisiteLog?url="+url,"API分时统计")
        		}
    		});
    	})
    </script>
