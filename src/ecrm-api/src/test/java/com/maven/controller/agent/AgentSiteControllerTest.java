...
public class AgentSiteControllerTest extends TestCase {
	...
	public void testAgentDomain() {
		try {
			String url = "http://192.168.1.207:8080/ecrm-api/Agent/AgentDomain?domain=openstack.com";
			System.out.println("代理站点模板信息:"+url);
			System.out.println(EntityUtils.toString(httpclient.execute(new HttpGet(url)).getEntity()));
		} catch (Exception e) {
			e.printStackTrace();
			fail(e.getMessage());
		}
