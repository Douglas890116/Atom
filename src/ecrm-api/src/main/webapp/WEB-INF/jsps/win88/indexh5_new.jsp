...
<script type="text/javascript" src="https://login.luckydragon88.com/jswrapper/integration.js.php?casino=greenjade88"></script>
...
if(gametype != "") {
	...
	<body>
		...
		<div class="mui-content">
			...
			<div class="search">
	    		<form name="frm1" id="frm1" method="post" action="${ctx}/win88game/indexh5">
		    ...
		    <div class="mui-scroll-wrapper mui-slider-indicator mui-segmented-control mui-segmented-control-inverted scroll_row">	
		        <div class="mui-scroll">
		            ...
		            <c:if test="${stype == '最新游戏' }"><div class="mui-control-item mui-active" onclick="javascript:location='${ctx}/win88game/indexh5?stype=<%=AESUtil.encrypt("最新游戏") %>'">最新游戏</div></c:if>
                	<c:if test="${stype != '最新游戏' }"><div class="mui-control-item"            onclick="javascript:location='${ctx}/win88game/indexh5?stype=<%=AESUtil.encrypt("最新游戏") %>'">最新游戏</div></c:if>
                
                	<c:if test="${stype == '热门游戏' }"><div class="mui-control-item mui-active" onclick="javascript:location='${ctx}/win88game/indexh5?stype=<%=AESUtil.encrypt("热门游戏") %>'">热门游戏</div></c:if>
                	<c:if test="${stype != '热门游戏' }"><div class="mui-control-item"            onclick="javascript:location='${ctx}/win88game/indexh5?stype=<%=AESUtil.encrypt("热门游戏") %>'">热门游戏</div></c:if>
                
                	<c:if test="${stype == '全部游戏' }"><div class="mui-control-item mui-active" onclick="javascript:location='${ctx}/win88game/indexh5?stype=<%=AESUtil.encrypt("全部游戏") %>'">全部游戏</div></c:if>
                	<c:if test="${stype != '全部游戏' }"><div class="mui-control-item"            onclick="javascript:location='${ctx}/win88game/indexh5?stype=<%=AESUtil.encrypt("全部游戏") %>'">全部游戏</div></c:if>
