...
<c:set var="statics" value="${ctx}/static/qt2/"></c:set>
...
<script type="text/javascript">
	...
	<body>
		<div class="top">
	        ...
	        <div class="top_right">
	        	<form name="frm1" id="frm1" method="post" action="${ctx}/qtgame/index">
		...
		<div class="games">
			...
			<ol class="gameN hidden-xs">
				<c:if test="${stype == '最新发布' }"><li class="on" onclick="javascript:location='${ctx}/qtgame/index?stype=<%=AESUtil.encrypt("最新发布") %>'">最新发布</li></c:if>
                <c:if test="${stype != '最新发布' }"><li            onclick="javascript:location='${ctx}/qtgame/index?stype=<%=AESUtil.encrypt("最新发布") %>'">最新发布</li></c:if>
                
                <c:if test="${stype == '热门游戏' }"><li class="on" onclick="javascript:location='${ctx}/qtgame/index?stype=<%=AESUtil.encrypt("热门游戏") %>'">热门游戏</li></c:if>
                <c:if test="${stype != '热门游戏' }"><li            onclick="javascript:location='${ctx}/qtgame/index?stype=<%=AESUtil.encrypt("热门游戏") %>'">热门游戏</li></c:if>
                
                <c:if test="${stype == '老虎机' }"><li class="on" onclick="javascript:location='${ctx}/qtgame/index?stype=<%=AESUtil.encrypt("老虎机") %>'">老虎机</li></c:if>
                <c:if test="${stype != '老虎机' }"><li            onclick="javascript:location='${ctx}/qtgame/index?stype=<%=AESUtil.encrypt("老虎机") %>'">老虎机</li></c:if>
                
                <c:if test="${stype == '桌面游戏' }"><li class="on" onclick="javascript:location='${ctx}/qtgame/index?stype=<%=AESUtil.encrypt("桌面游戏") %>'">桌面游戏</li></c:if>
                <c:if test="${stype != '桌面游戏' }"><li            onclick="javascript:location='${ctx}/qtgame/index?stype=<%=AESUtil.encrypt("桌面游戏") %>'">桌面游戏</li></c:if>
			...
			<div class="container">
				<div class="row">
					...
					<div class="col-xs-5 col-sm-3 col-md-2 col-lg-2 tbl" >
						<img data-original="${statics }/game/${item.imagename }.jpg" alt="" src="${statics }/game/qt.jpg" class="gameimg"/><h5>${item.cnname}</h5>
						<ul>
						<li onclick="window.open('${ctx }/qtgame/login?gamecode=${item.gamecodeweb }')">开始游戏</li>
