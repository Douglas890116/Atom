...
<c:set var="statics" value="${ctx}/static/hab2/"></c:set>
...
<script type="text/javascript">
	...
	<body>
		<div class="top">
	        ...
	        <div class="top_right">
	        	<form name="frm1" id="frm1" method="post" action="${ctx}/habgame/index">
		...
		<div class="games">
			...
			<ol class="gameN hidden-xs">
				<c:if test="${stype == '最新游戏' }"><li class="on" onclick="javascript:location='${ctx}/habgame/index?stype=<%=AESUtil.encrypt("最新游戏") %>'">最新游戏</li></c:if>
                <c:if test="${stype != '最新游戏' }"><li            onclick="javascript:location='${ctx}/habgame/index?stype=<%=AESUtil.encrypt("最新游戏") %>'">最新游戏</li></c:if>
                
                <c:if test="${stype == '热门游戏' }"><li class="on" onclick="javascript:location='${ctx}/habgame/index?stype=<%=AESUtil.encrypt("热门游戏") %>'">热门游戏</li></c:if>
                <c:if test="${stype != '热门游戏' }"><li            onclick="javascript:location='${ctx}/habgame/index?stype=<%=AESUtil.encrypt("热门游戏") %>'">热门游戏</li></c:if>
                
                <c:if test="${stype == '澳门游戏' }"><li class="on" onclick="javascript:location='${ctx}/habgame/index?stype=<%=AESUtil.encrypt("澳门游戏") %>'">澳门游戏</li></c:if>
                <c:if test="${stype != '澳门游戏' }"><li            onclick="javascript:location='${ctx}/habgame/index?stype=<%=AESUtil.encrypt("澳门游戏") %>'">澳门游戏</li></c:if>
                
                <c:if test="${stype == '桌面纸牌' }"><li class="on" onclick="javascript:location='${ctx}/habgame/index?stype=<%=AESUtil.encrypt("桌面纸牌") %>'">桌面纸牌</li></c:if>
                <c:if test="${stype != '桌面纸牌' }"><li            onclick="javascript:location='${ctx}/habgame/index?stype=<%=AESUtil.encrypt("桌面纸牌") %>'">桌面纸牌</li></c:if>
                
                <c:if test="${stype == '视频扑克' }"><li class="on" onclick="javascript:location='${ctx}/habgame/index?stype=<%=AESUtil.encrypt("视频扑克") %>'">视频扑克</li></c:if>
                <c:if test="${stype != '视频扑克' }"><li            onclick="javascript:location='${ctx}/habgame/index?stype=<%=AESUtil.encrypt("视频扑克") %>'">视频扑克</li></c:if>
                
                <c:if test="${stype == '全部游戏' }"><li class="on" onclick="javascript:location='${ctx}/habgame/index?stype=<%=AESUtil.encrypt("全部游戏") %>'">全部游戏</li></c:if>
                <c:if test="${stype != '全部游戏' }"><li            onclick="javascript:location='${ctx}/habgame/index?stype=<%=AESUtil.encrypt("全部游戏") %>'">全部游戏</li></c:if>
			...
			<div class="container">
				<div class="row">
					...
					<div class="col-xs-5 col-sm-3 col-md-2 col-lg-2 tbl" >
						<img data-original="${statics}/game/${item.gamecodeweb}.png" alt="" src="${statics }/haba.jpg" class="gameimg"/><h5>${item.cnname}</h5>
						<ul>
						<li onclick="window.open('${ctx }/habgame/login?gamecode=${item.gamecodeweb}')">开始游戏</li>
