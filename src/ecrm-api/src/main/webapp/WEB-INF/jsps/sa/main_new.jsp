...
<c:set var="statics" value="${ctx}/static/sa2/"></c:set>
...
<script type="text/javascript">
	...
	<body>
		<div class="top">
	        ...
	        <div class="top_right">
	        	<form name="frm1" id="frm1" method="post" action="${ctx}/sagame/main">
		...
		<div class="games">
			...
			<ol class="gameN hidden-xs">
				<c:if test="${stype == '奖金游戏' }"><li class="on" onclick="javascript:location='${ctx}/sagame/main?stype=<%=AESUtil.encrypt("奖金游戏") %>'">奖金游戏</li></c:if>
                <c:if test="${stype != '奖金游戏' }"><li            onclick="javascript:location='${ctx}/sagame/main?stype=<%=AESUtil.encrypt("奖金游戏") %>'">奖金游戏</li></c:if>
                
                <c:if test="${stype == '全部游戏' }"><li class="on" onclick="javascript:location='${ctx}/sagame/main?stype=<%=AESUtil.encrypt("全部游戏") %>'">全部游戏</li></c:if>
                <c:if test="${stype != '全部游戏' }"><li            onclick="javascript:location='${ctx}/sagame/main?stype=<%=AESUtil.encrypt("全部游戏") %>'">全部游戏</li></c:if>
			...
			<div class="container">
				<div class="row">
					...
					<div class="col-xs-5 col-sm-3 col-md-2 col-lg-2 tbl" >
						<img data-original="${statics }/game/${item.imagename }.jpg" alt="" src="${statics }/game/sa.jpg" class="gameimg"/><h5>${item.cnname}</h5>
						<ul>
						<li onclick="window.open('${ctx }/sagame/login?gamecode=${item.gamecodeweb }')">开始游戏</li>
