...
<html>
	<head>
		...
		<!-- https://login.luckydragon88.com/jswrapper/integration.js.php?casino=greatfortune88
		https://login.greatfortune88.com/jswrapper/integration.js.php?casino=greatfortune88 -->
...
<script type="text/javascript" src="https://login.greatfortune88.com/jswrapper/integration.js.php?casino=greatfortune88"></script>
...
function testGame(idx)
 {
	...
	<body>
		<div class="top">
	        ...
	        <div class="top_right">
	        	<form name="frm1" id="frm1" method="post" action="${ctx}/ptgame/ptnew">
		...
		<div class="games">
			...
			<ol class="gameN hidden-xs">
				<c:if test="${stype == '最新游戏' }"><li class="on" onclick="javascript:location='${ctx}/ptgame/ptnew?stype=<%=AESUtil.encrypt("最新游戏") %>'">最新游戏</li></c:if>
                <c:if test="${stype != '最新游戏' }"><li            onclick="javascript:location='${ctx}/ptgame/ptnew?stype=<%=AESUtil.encrypt("最新游戏") %>'">最新游戏</li></c:if>
                
                <c:if test="${stype == '热门游戏' }"><li class="on" onclick="javascript:location='${ctx}/ptgame/ptnew?stype=<%=AESUtil.encrypt("热门游戏") %>'">热门游戏</li></c:if>
                <c:if test="${stype != '热门游戏' }"><li            onclick="javascript:location='${ctx}/ptgame/ptnew?stype=<%=AESUtil.encrypt("热门游戏") %>'">热门游戏</li></c:if>
                
                <c:if test="${stype == '全部游戏' }"><li class="on" onclick="javascript:location='${ctx}/ptgame/ptnew?stype=<%=AESUtil.encrypt("全部游戏") %>'">全部游戏</li></c:if>
                <c:if test="${stype != '全部游戏' }"><li            onclick="javascript:location='${ctx}/ptgame/ptnew?stype=<%=AESUtil.encrypt("全部游戏") %>'">全部游戏</li></c:if>
